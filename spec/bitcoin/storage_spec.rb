...
#  { :name => :sequel, :db => 'postgres://localhost/bitcoin_test' },
  ...
  describe "Bitcoin::Storage::Backends::#{configuration[:name].capitalize}Store" do
    ...
    describe "reorg" do
      ...
      it "should reconnect an orphan" do
        block1 = create_block @block0.hash, false
        block2 = create_block block1.hash, false
        @store.store_block(block2)
        @store.get_head.should == @block0
        @store.get_block(block2.hash).chain.should == 2
        @store.store_block(block1)
        @store.get_head.should == block2
      end
    end
  end
end
