...
module Bitcoin
  ...
  Integer = # rubocop:disable Naming/ConstantName
  ...
  module Util
    ...
    def address_type(address)
      ...
      if segwit_decoded
        ...
        return :witness_v0_keyhash if witness_version == 0 && witness_program.bytesize == 20

        return :witness_v0_scripthash if witness_version == 0 && witness_program.bytesize == 32
      end
    ...
    def decode_compact_bits(bits) # rubocop:disable Metrics/PerceivedComplexity
    ...
    def sign_data(key, data)
      ...
      loop do
        ...
        p [
          ...
          data.unpack('H*')[0], sig.unpack('H*')[0]
        ]
      end
      sig
    end
    ...
    def open_key(private_key, public_key = nil)
      ...
      public_key ||= regenerate_public_key(private_key)
      key.public_key = ::OpenSSL::PKey::EC::Point.from_hex(key.group, public_key)
      key
    end
    ...
    def block_difficulty(target_nbits)
      ...
      difficulty = Math.exp(max_body -
                            Math.log(bits & 0x00ffffff) +
                            scaland *
                            (0x1d - ((bits & 0xff000000) >> 24)))
      format('%.7f', difficulty)
    end
    ...
    def block_probability(target_nbits)
      ...
      format('%.55f', (current_target.to_f / max_target))
    end
    ...
    def block_creation_reward(block_height)
      scaling_factor = (2**(block_height / Bitcoin.network[:reward_halving].to_f).floor)
      Bitcoin.network[:reward_base] / scaling_factor
    end
  end

  extend Util

  # Binary helpers/extensions
  ...
  # Binary extensions to String class
  class ::String # rubocop:disable Style/ClassAndModuleChildren
    include Bitcoin::BinaryExtensions
  end

  module ::OpenSSL # rubocop:disable Style/ClassAndModuleChildren
    # BigNumber
    class BN
    ...
    module PKey
      class EC
        ...
        class Point
          ...
          def ec_add(point)
            self.class.new(group, OpenSSL::BN.from_hex(OpenSSL_EC.ec_add(self, point)))
          end
        end
      end
    end
