...
module Bitcoin
  module Util
    ...
    def hash_mrkl_tree(tx)
      ...
      t = tx.each_slice(2).map{|i|
        tree << i
        Bitcoin.bitcoin_mrkl( i[0], i[1] || i[0] )
      }
      t << t.each_slice(2).map{|i|
        Bitcoin.bitcoin_mrkl( i[0], i[1] || i[0] )
      }

      tree.flatten + t.flatten
    end
  end
...
if $0 == __FILE__
  ...
  describe 'Bitcoin Utils' do
    ...
    it '#block_hash' do
      ...
      # block 198
      prev_block="00000000ef271937f59286dc81d6b28196ff099bf266e12a3442c4b82c9db814"
      mrkl_root ="8453119b70f9a2b31aa32c64b5cade25fea8b67fb3437cb90a2332463f03250c"
      time, bits, nonce, ver = 1231752161, 486604799, 798850333, 1
      Bitcoin.block_hash(prev_block, mrkl_root, time, bits, nonce, ver).should ==
        "00000000e85458c1467176b04a65d5efaccfecaaab717b17a587b4069276e143"

    end

    it 'hash mrkl_tree from transactions' do
      tx = [
      ...
      Bitcoin.hash_mrkl_tree(tx).should == mrkl_tree

      Bitcoin.bitcoin_mrkl( tx[0], tx[1] ).should == mrkl_tree[3]
      Bitcoin.bitcoin_mrkl( tx[2], tx[2] ).should == mrkl_tree[4]
...
__END__
# first block: http://blockexplorer.com/rawblock/000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f

=begin
require 'pp'

#p Bitcoin.generate_key
#pp (0...2).map{ Bitcoin.generate_address }
#pp (0...250).map{ Bitcoin.generate_address }
#pp (0...250).map{ Bitcoin.generate_address.map(&:size) }.uniq

def find_address
  look = ['arch', 'ruby']
  found, chunks = false, 0
  while !found
    (0...250).each{
      a = Bitcoin.generate_address
      address = a.first.downcase
      if look.any?{|i| address.include?(i) }
        found = a
      end
      sleep(0.001)
    }
...
#msg_hash = OpenSSL::Digest::SHA1.digest("hello")
#p signature = [key.dsa_sign_asn1(msg_hash)].pack('m')
=end
